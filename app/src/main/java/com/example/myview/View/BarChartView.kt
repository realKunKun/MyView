...
class BarChartView(context: Context, attr: AttributeSet?, defStyleAttr:Int):View(context,attr,defStyleAttr) {
    ...
    init {
        ...
        XMaxTextRect= Rect()
        YMaxTextRect= Rect()
        TextSize= array.getDimension(R.styleable.TheFirstView_text_size, TextSize.toFloat()).toInt()
        initPaint(false)
        //注意资源回收
        array.recycle()
    }
    ...
    //绘制文字的画笔
    paintText.getTextBounds((yAxisList[yAxisList.size-1]).toString(),0,(yAxisList[yAxisList.size-1]).toString().length,YMaxTextRect)
    paintText.getTextBounds(xAxisList[xAxisList.size-1],0, xAxisList[xAxisList.size-1].length,XMaxTextRect)
    paintText.textSize=50F
    //绘制的刻度文字的最大值所占的高宽
        maxTextWidth=
            if (YMaxTextRect.width() > XMaxTextRect.width()) YMaxTextRect.width() else XMaxTextRect.width()
        maxTextHeight =
            if (YMaxTextRect.height() > XMaxTextRect.height()) YMaxTextRect.height() else XMaxTextRect.height()

    if (yAxisList.size>=2) valueSpace=yAxisList[1]-yAxisList[0]
    startX=maxTextWidth+keduWidth+keduTextSpace
    startY=keduSpace*(yAxisList.size-1)+maxTextHeight+if (isShowValueText) keduTextSpace else 0
    }
    fun initData(){
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (heightMode==MeasureSpec.AT_MOST){
            heightSize = if (keduWidth>maxTextHeight+keduTextSpace) (yAxisList.size-1)*keduSpace+keduWidth+maxTextHeight
                        else (yAxisList.size-1)*keduSpace+(maxTextHeight+keduTextSpace)+maxTextHeight
            heightSize += keduTextSpace + if (isShowValueText) keduTextSpace else 0
        }
        if (widthMode==MeasureSpec.AT_MOST)widthSize=startX+Data.size*itemWidth+(Data.size+1)*itemSpace
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawLine(startX.toFloat(), startY.toFloat()+keduWidth,
            startX.toFloat(), (startY-(yAxisList.size-1)*keduSpace).toFloat(),paintLine)
        for (i in 0 until yAxisList.size) {
            ...
            canvas.drawLine(
                ...
                (startX + Data.size * itemWidth + itemSpace * (Data.size + 1)).toFloat(),
                (startY - keduSpace * i).toFloat(),
                paintLine
            )
        }
        ...
        for (j in 0 until xAxisList.size){
            ...
            if (isShowValueText) {
                ...
                canvas!!.drawText(
                    ...
                    (startY - keduTextSpace - Data[j] * (keduSpace * 1.0 / valueSpace)).toFloat(),
                    paintText
                )
            }
            //绘制柱状条
            ...
            canvas!!.drawRect(
                initx.toFloat(), (startY - Data[j] * (keduSpace * 1.0 / valueSpace)).toFloat(),
                (initx + itemWidth).toFloat(), startY.toFloat(), paintBar
            )
        }
    }
    fun updateValueData(datas: ArrayList<Int>, xList: ArrayList<String>, yList: ArrayList<Int>) {
        Data = datas
        xAxisList = xList
        yAxisList = yList
        initPaint( true)
